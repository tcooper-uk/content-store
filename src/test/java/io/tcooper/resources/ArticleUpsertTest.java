...
public class ArticleUpsertTest {
  ...
  void setup() {
    ...
    testArticle = new Article(new ArticleUid(), name, description, content, page);
  }
  ...
  void givenValidArticleThenValidResponse(){
    ...
    assertThat(articleInsertResponse.isCreated()).isTrue();
  ...
  @Test
  void givenValidArticleToUpdateThenValidResponse() {

    Mockito.when(mongoCollectionMock.findOneAndReplace(isA(Bson.class), isA(Article.class)))
        .thenReturn(testArticle);

    Response response = underTest.target("/article/942ee57d-7a4f-43fc-91f9-b474d8972ae0")
        .request()
        .buildPost(Entity.entity(articleInsertRequest, MediaType.APPLICATION_JSON))
        .invoke();
    ...
    verify(mongoCollectionMock, times(1)).findOneAndReplace(isA(Bson.class), isA(Article.class));
  }
  ...
  @Test
  void givenInvalidArticleForUpdateThenValidErrors() {
    Response response = underTest.target("/article/942ee57d-7a4f-43fc-91f9-b474d8972ae0")
        .request()
        .buildPost(Entity.entity(new ArticleInsertRequest(), MediaType.APPLICATION_JSON))
        .invoke();
